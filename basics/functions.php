...
<?php
	...
	global $conexion;
	$consulta= "SELECT E.id, E.nombre, P.estado, P.nota, P.inicio, P.terminada, E.activo, E.tiempo
				FROM pruebas P
	...
	$consulta= //-----
				...
				ORDER BY nombre"; 
				//-----
    $resultado = mysql_query($consulta,$conexion) or die(mysql_error()); ; // ejecucion de la consulta anterior
	$valor = mysql_fetch_array($resultado);
		return $valor ;
}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
							...
							$idCliente, $idSolicitud)
	{
		...
		$consultaOffline = "SELECT *
							FROM ex_solicitados
							WHERE id = '{$idSolicitud}'
							LIMIT 1";
		$resultadoOffline = mysql_query($consultaOffline,$conexion);
		verificar_consulta($resultadoOffline);
		$examenOffline = mysql_fetch_array($resultadoOffline);
		
		if ($examenOffline['tipoDeExamen'] != "Offline")
		{
...
/////////////////////////////////////////////////////////////////PRODUCTOS CLIENTES///////////////////////////////////////////////////////////////////////////////////
	function ex_codigos_idCliente($id_examen)
	{
		global $conexion;
		$consulta = "SELECT M.materia, E.nombre, C.codigoExamen, C.passwordExamen
					FROM ex_clientes EC
					INNER JOIN examenes E
					ON EC.id_examen = E.id
					INNER JOIN materias M
					ON E.id_materia = M.idMateria
					INNER JOIN ex_codigos C
					ON EC.id_examen = C.idExamen
					WHERE EC.id_examen ='{$id_examen}'";
		$resultado = mysql_query($consulta,$conexion);
		verificar_consulta($resultado); 
		return $resultado;
	}
	...
	function l_alumnos_usuarios($alumno_id){ 				
	global $conexion;
	$consulta= //-----
				"SELECT * 
				FROM usuarios 
	...
	function l_alumno_prueba($alumno_id){ 				
	global $conexion;
	$consulta= //-----
				"SELECT * 
				FROM pruebas 
	...
	function l_preguntas($id_examen, $id_pregunta){
	global $conexion;
	$consulta= //-----
				"SELECT tipoPregunta
				FROM preguntas
				WHERE idExamen = '{$id_examen}'
				AND idPregunta = '{$id_pregunta}'"; 
				//-----
    $resultado = mysql_query($consulta,$conexion);
	return $resultado ;
	}
...
/////////////////////////////////////////////////////////////////EXAMEN ALUMNO///////////////////////////////////////////////////////////////////////////////////
	...
	function l_preguntas_idExamen($id_examen)
	{
		global $conexion;
		$consulta= "SELECT *
					FROM preguntas
	...
	function cargarResultadoExamen($idExamen, $idAlumno, $idPregunta, $idRespuesta, $correcta)
	{	
		global $conexion;
		$consulta= "INSERT INTO respuestas (idExamen, idAlumno, idPregunta, idRespuesta, correcta)
					VALUES ('{$idExamen}', '{$idAlumno}', '{$idPregunta}', '{$idRespuesta}', '{$correcta}')";
		mysql_query($consulta,$conexion);
	}
		function l_examenes_online($id_cliente)
	{	
		global $conexion;
		$consulta=	"SELECT E.id, E.nombre, E.descripcion, E.tiempo, ES.tipoDeExamen
					FROM examenes E
					INNER JOIN ex_solicitados ES
							ON E.id = ES.idExamen
					WHERE ES.idCliente = '{$id_cliente}' ";
		$resultado = mysql_query($consulta,$conexion)or die(mysql_error()); 
		return $resultado ;
	}
	function l_pruebas_alumno_examen($id_prueba, $id_alumno){
	global $conexion;
	$consulta= "SELECT * 
				FROM pruebas 
    ...
    $resultado = mysql_query($consulta,$conexion) or die(mysql_error());  // ejecucion de la consulta anterior
		...
		function l_respuestas($id_examen, $id_alumno, $id_pregunta)
	{
		global $conexion;
		$consulta= "SELECT *
					FROM respuestas
	...
	function l_nom_respuesta($id_examen, $id_pregunta)
	{
		global $conexion;
		$consulta= "SELECT *
					FROM preguntas
	...
	function buscar_tiempo($id_examen)
	{	
		global $conexion;
		$consulta =	"SELECT tiempo
					FROM examenes
					WHERE id = '{$id_examen}' ";
		$resultado = mysql_query($consulta,$conexion); 
		$tiempo = mysql_fetch_array($resultado);
		return $tiempo ;
	}
	
	function insertarInicioExamen($id_examen, $id_alumno)
	{	
		global $conexion;
		$update= "UPDATE pruebas 
				  SET inicio = now()
	...
	function insertarFinExamen($nota, $id_examen, $id_alumno)
	{	
		global $conexion;
		$update= "UPDATE pruebas 
				  SET nota = '{$nota}', estado = 1, terminada = now()
				  WHERE id_examen = '{$id_examen}'
				    AND id_alumno = '{$id_alumno}'"; 
		$resultado = mysql_query($update,$conexion);
		verificar_consulta($resultado);
	...
	function l_pruebas_examen_alumno($id_alumno, $id_examen){
	global $conexion;
	$consulta= "SELECT E.id, E.nombre, P.estado, P.nota, P.inicio, P.terminada, E.activo, E.tiempo
				FROM pruebas P
				INNER JOIN examenes E
	...
	function l_resultados($id_alumno, $id_examen){
	global $conexion;
	$consulta= "SELECT P.pregunta, P.resp1, P.resp2, P.resp3, P.resp4, P.resp5, P.respcorrecta, P.tipoPregunta, R.idAlumno, R.idRespuesta, R.correcta
				FROM preguntas P
				INNER JOIN respuestas R
				        ON P.idExamen = R.idExamen
				WHERE R.idAlumno = '{$id_alumno}'
				  AND P.idExamen = '{$id_examen}' "; 
    $resultado = mysql_query($consulta,$conexion); // ejecucion de la consulta anterior
	return $resultado ;
	}
